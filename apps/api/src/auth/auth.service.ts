...
export class AuthService {
    ...
    async authenticate(loginRequest: LoginRequest, res: any): Promise<LoginResponse> {
        ...
        if (success) {
            const payload = { id: userLogin.id, email: userLogin.email, name: userLogin.name, isAdmin: userLogin.isAdmin };
            const sessionToken = await this.jwtService.signAsync(payload)
            res.cookie(AuthService.SESSION_COOKIE_KEY, { sessionToken: sessionToken, isAdmin: userLogin.isAdmin });
            return { sessionToken, isAuthenticated: true, isAdmin: userLogin.isAdmin };
        }
